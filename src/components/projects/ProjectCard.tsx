...
export function ProjectCard({
  ...
  isClicked, // Prop to manage visual state
}: {
  name: string;
  role: string;
  avatar: string;
  ...
  return (
    <div
      ...
      className={`relative w-full h-full rounded-lg overflow-hidden border ${
        isClicked ? 'border-yellow-500 shadow-lg' : 'border-transparent'
      } hover:border-yellow-500 hover:shadow-lg shadow-gray-700 transition-all duration-300`}
    >
      {/* Center and maintain aspect ratio of avatar image */}
      <div
        className={`absolute size-28 mt-40 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-full bg-gray-800 overflow-hidden ${
          isClicked ? 'ring-4 ring-yellow-500' : ''
        }`}
      >
        <img
          src={avatar}
          alt={name}
          className={`w-full h-full object-cover ${isClicked ? 'animate-pulse' : ''}`}
        />
      </div>
